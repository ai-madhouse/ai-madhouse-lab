...
- Markdown rendering is intentionally lightweight and safe (no HTML passthrough, no `dangerouslySetInnerHTML`), so unsupported markdown syntax will display as plain text.
...
- If richer markdown compatibility is required later (nested lists, task lists, tables, escaped markers, full emphasis nesting), consider replacing the custom parser with a vetted markdown pipeline and an allowlist sanitizer.
